...
def _inferExprBatched(truthBatch, truth, dialect, expr, maxLen=MAX_LENGTH):
    ...
    try:
        length = 0
        for chunk in _chunks(list(range(1, maxLen + 1)), BATCH_SIZE):
        ...
        for pos in range(1, length + 1):
            if pos in wide:
                ...
                value += _chr(code) if CHAR_MIN <= code <= CHAR_MAX else "?"
    except InconclusiveError:
...
def _catList(infer, dialect, col, fromClause, label="catalog"):
    ...
    names, missing = [], 0
    for offset in range(min(count, DUMP_MAX_ROWS)):
            ...
            try:
                ...
                for ci, col in enumerate(columns):
                    if col in kb.data.dumpedTable:
                        ...
                        for ri in range(min(len(rows), len(vals))):
                            if ci < len(rows[ri]):
                                rows[ri][ci] = vals[ri]
            except Exception:
                pass
