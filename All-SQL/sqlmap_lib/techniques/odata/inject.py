...
_FREQ = (tuple(range(ord('a'), ord('z') + 1)) + tuple(range(ord('A'), ord('Z') + 1)) +
         tuple(range(ord('0'), ord('9') + 1)) + tuple(ord(_) for _ in " @._-+:/!#$%&*=?"))
_CS_ORDS = []
for _o in _FREQ:
    if ODATA_CHAR_MIN <= _o <= ODATA_CHAR_MAX and _o not in _CS_ORDS:
        _CS_ORDS.append(_o)
for _o in range(ODATA_CHAR_MIN, ODATA_CHAR_MAX + 1):
...
def _inferField(oracle, key, keyValue, field, maxLen=ODATA_MAX_LENGTH, useIn=None):
    ...
    try:
        ...
        for pos in range(length):
            ...
            recovered = "?"
            for start in range(0, len(_CS_ORDS), ODATA_CHARSET_BLOCK):
