...
_QUERY = re.compile(r'[?&]([A-Za-z_][A-Za-z0-9_]*)=[r"\']?(\d{1,9})(?!\d)')
...
_INAME = re.compile(r'\bname\s*=\s*[r"\']?([A-Za-z_][A-Za-z0-9_]*)', re.I)
...
def _chunks(length, perCall):
    ...
    return [(1 + _, min(perCall, length - _)) for _ in range(0, length, perCall)]
...
def _serial(profile, expression, chunks, perCall):
    ...
    for position, count in chunks or ((_, perCall) for _ in range(1, MAX_MULTIBIT_LENGTH, perCall)):
