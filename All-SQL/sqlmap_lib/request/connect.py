...
class Connect(object):
    ...
    def getPage(**kwargs):
        ...
        if conf.dummy:
            page, headers, code = randomStr(int(randomInt()), alphabet=[_chr(_) for _ in range(256)]), None, None
            threadData.lastPage = page
            threadData.lastCode = code
            return page, headers, code

        # Jitter (--jitter=N, testing): ~1 in N requests is perturbed to stress the time-/
        ...
        if conf.liveCookies:
            with kb.locks.liveCookies:
                if not checkFile(conf.liveCookies, raiseOnError=False) or os.path.getsize(conf.liveCookies) == 0:
                    ...
                    valid = False
                    for _ in range(LIVE_COOKIES_TIMEOUT):
            ...
            if webSocket:
                ...
                try:
                    ...
                    if kb.webSocketRecvCount is None:
                        ...
                        for i in range(max(1, kb.webSocketRecvCount)):
    ...
    def queryPage(value=None, place=None, content=False, getRatioValue=False, silent=False, method=None, timeBasedCompare=False, noteResponseTime=True, auxHeaders=None, response=False, raise404=None, removeReflection=True, disableTampering=False, ignoreSecondOrder=False, buildOnly=False):
        ...
        if conf.csrfToken:
            ...
            for attempt in range(conf.csrfRetries + 1):
                if token:
                    break
                ...
                if match:
                    ...
                    if match:
                        token.value = "".join(_chr(int(_)) for _ in match.group(1).replace(' ', "").split(','))
