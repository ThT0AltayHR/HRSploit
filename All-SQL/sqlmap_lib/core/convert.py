...
def getBytes(value, encoding=None, errors="strict", unsafe=True):
    ...
    True
    >>> getBytes(ur"C:\\\\\\\\x64\\\\secrets.txt") == br"C:\\\\x64\\\\secrets.txt"
    True
    """
        ...
        if INVALID_UNICODE_PRIVATE_AREA:
            if unsafe:
                for char in range(0xF0000, 0xF00FF + 1):
                    value = value.replace(_chr(char), "%s%02x" % (SAFE_HEX_MARKER, char - 0xF0000))
...
def getUnicode(value, encoding=None, noneToNull=False):
    ...
    if isinstance(value, six.text_type):
        ...
        try:
            ...
            return six.text_type(str(value), errors="ignore")  # encoding ignored for non-str instances
