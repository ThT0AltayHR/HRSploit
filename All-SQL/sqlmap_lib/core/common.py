...
def jsonMinimize(content):
    ...
    def _walk(obj, path):
        if isinstance(obj, dict):
            ...
            for index in range(len(obj)):                         # index kept -> order-sensitive (correct for result sets)
...
def predictValue(prevValue, originalCharset):
    ...
    if kb.partRun in kb.commonOutputs:
        ...
        for ordChar in originalCharset:
            if _chr(ordChar) not in predictionSet:
...
def unArrayizeValue(value):
    ...
    'a'
    >>> unArrayizeValue(_ for _ in range(10))
    0
    """
        ...
        try:
            if value > 255:
                ...
                if Backend.isDbms(DBMS.MYSQL):
                    ...
                    retVal = _chr(value) if value > 0xFFFF else getUnicode(raw, "UTF-16-BE")
