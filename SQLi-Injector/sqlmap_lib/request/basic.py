    ...
    if isinstance(page, six.binary_type) and "text/" in contentType:
        if not kb.disableHtmlDecoding:
            ...
            if b"&#" in page:
                ...
                page = re.sub(br"&#(\\d{1,3});", lambda _: six.int2byte(int(_.group(1))) if int(_.group(1)) < 256 else _.group(0), page)
            ...
            if "&#" in page:
                ...
                def _(match):
                    retVal = match.group(0)
                    try:
                        retVal = _chr(int(match.group(1), 16))
                    except (ValueError, OverflowError):
                        pass
                    return retVal
            ...
            # e.g. &zeta;
            page = re.sub(r"&([^;]+);", lambda _: _chr(HTML_ENTITIES[_.group(1)]) if HTML_ENTITIES.get(_.group(1), 0) > 255 else _.group(0), page)
        else:
            page = getUnicode(page, kb.pageEncoding)
