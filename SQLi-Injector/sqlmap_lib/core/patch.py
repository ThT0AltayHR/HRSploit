...
def dirtyPatches():
    ...
    try:
        ...
        if six.PY3:
            ...
            os.urandom = lambda size: "".join(chr(random.randint(0, 255)) for _ in range(size))
    ...
    def _reversible(ex):
        if INVALID_UNICODE_PRIVATE_AREA:
            return (u"".join(_chr(int('000f00%02x' % (_ if isinstance(_, int) else ord(_)), 16)) for _ in ex.object[ex.start:ex.end]), ex.end)
        else:
