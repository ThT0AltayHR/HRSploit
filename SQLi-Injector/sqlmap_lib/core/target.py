        ...
        if re.search(JSON_RECOGNITION_REGEX, conf.data):
            ...
            if choice == 'Q':
                ...
                if not (kb.processUserMarks and kb.customInjectionMark in conf.data):
                    ...
                    conf.data = re.sub(r'("(?P<name>[^"]+)r"\s*:\s*".*?)"(?<!\\")', functools.partial(process, repl=r'\g<1>%s"' % kb.customInjectionMark), conf.data)
                    conf.data = re.sub(r'("(?P<name>[^"]+)r"\s*:\s*")"', functools.partial(process, repl=r'\g<1>%s"' % kb.customInjectionMark), conf.data)
                    conf.data = re.sub(r'("(?P<name>[^"]+)r"\s*:\s*)(-?\d[\d\.]*)\b', functools.partial(process, repl=r'\g<1>\g<3>%s' % kb.customInjectionMark), conf.data)
                    conf.data = re.sub(r'("(?P<name>[^"]+)r"\s*:\s*)((true|false|null))\b', functools.partial(process, repl=r'\g<1>\g<3>%s' % kb.customInjectionMark), conf.data)
                    for match in re.finditer(r'(?P<name>[^"]+)r"\s*:\s*\[([^\]]+)\]', conf.data):
                        if not (conf.testParameter and match.group("name") not in conf.testParameter):
                            ...
                            if kb.customInjectionMark not in _:  # Note: only for unprocessed (simple) forms - i.e. non-associative arrays (e.g. [1,2,3])
                                _ = re.sub(r'("[^"]+)"', r'\g<1>%sr"' % kb.customInjectionMark, _)
            ...
            if choice == 'Q':
                ...
                if not (kb.processUserMarks and kb.customInjectionMark in conf.data):
                    ...
                    conf.data = conf.data.replace(kb.customInjectionMark, ASTERISK_MARKER)
                    if '"' in conf.data:
                        conf.data = re.sub(r'((?P<name>"[^"]+r"|\w+)\s*:\s*"[^"]+)"', functools.partial(process, repl=r'\g<1>%s"' % kb.customInjectionMark), conf.data)
                        conf.data = re.sub(r'((?P<name>"[^"]+r"|\w+)\s*:\s*)(-?\d[\d\.]*\b)', functools.partial(process, repl=r'\g<0>%s' % kb.customInjectionMark), conf.data)
                    else:
                ...
                if not (kb.processUserMarks and kb.customInjectionMark in conf.data):
                    ...
                    def processAttributes(match):
                        def _attr(m):
                            ...
                            return "%s%s%s%s%sr" % (m.group(1), m.group(3), m.group(4), kb.customInjectionMark, m.group(5))
