...
def _whereDump(place, parameter, bound, threshold):
    ...
    for index in range(NOSQL_MAX_FIELDS):
        try:
...
def _cypherDump(place, parameter):
    ...
    try:
        for _ in range(NOSQL_MAX_RECORDS):
...
def _records(page):
    ...
    for body in re.findall(r"(?is)<table[^>]*>(.*?)</table>", page or ""):
        ...
        if rows:
            ...
            columns = header if header and len(header) == width else ["column_%d" % (_ + 1) for _ in range(width)]
...
def _grid(columns, rows):
    ...
    widths = [max([len(columns[index])] + [len(row[index]) for row in rows if index < len(row)]) for index in range(len(columns))]
    separator = '+' + '+'.join('-' * (width + 2) for width in widths) + '+'
    line = lambda cells: "| " + " | ".join((cells[index] if index < len(cells) else "").ljust(widths[index]) for index in range(len(columns))) + " |"
