...
_CS_ORDS = [_ for _ in range(XPATH_CHAR_MIN, XPATH_CHAR_MAX + 1) if _ not in _META_ORDS]
...
_FILE_ORDS = [0x09, 0x0a] + [_ for _ in range(XPATH_CHAR_MIN, XPATH_CHAR_MAX + 1)]
...
def _walkTree(oracle, builder, path="/*", depth=0):
    ...
    attributes = []
    for i in range(1, (attrCount or 0) + 1):
    ...
    children = []
    for i in range(1, (childCount or 0) + 1):
...
def _grid(columns, rows):
    ...
    def line(cells):
        return "| " + " | ".join((getUnicode(cells[index]) if index < len(cells) else "").ljust(widths[index]) for index in range(len(columns))) + " |"
