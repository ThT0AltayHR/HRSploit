"""
TiDB Database Dump Module - Dosya 63
...
logger = logging.getLogger('TiDBDumper')
...
class TiDBImplementation(TiDBDumper):
    """Concrete implementation for TiDB"""
    ...
    def get_tables(self) -> List[str]:
        """Get tables from TiDB"""
        if not self.connection:
            return []
        
        tables = []
        try:
            # Get tables query for TiDB
...
class TiDBDumpManager:
    ...
    def create_dump(self, config: DumpConfig) -> str:
        """Create new dump"""
        with self.lock:
            dumper = TiDBImplementation(config)
