"""
Hazelcast Database Dump Module - Dosya 47
...
logger = logging.getLogger('HazelcastDumper')
...
class HazelcastImplementation(HazelcastDumper):
    """Concrete implementation for Hazelcast"""
    
    def __init__(self, config: DumpConfig):
        super().__init__(config)
        self.db_type = 'Hazelcast'
        
    def connect(self) -> bool:
        """Connect to Hazelcast"""
        try:
            # Connection logic for Hazelcast
    ...
    def get_tables(self) -> List[str]:
        """Get tables from Hazelcast"""
        if not self.connection:
            return []
        
        tables = []
        try:
            # Get tables query for Hazelcast
...
class HazelcastDumpManager:
    """Manage Hazelcast dumps"""
    ...
    def create_dump(self, config: DumpConfig) -> str:
        """Create new dump"""
        with self.lock:
            dumper = HazelcastImplementation(config)
