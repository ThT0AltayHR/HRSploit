"""
Memsql Database Dump Module - Dosya 55
...
logger = logging.getLogger('MemsqlDumper')
...
class MemsqlImplementation(MemsqlDumper):
    """Concrete implementation for Memsql"""
    ...
    def get_tables(self) -> List[str]:
        """Get tables from Memsql"""
        if not self.connection:
            return []
        
        tables = []
        try:
            # Get tables query for Memsql
...
class MemsqlDumpManager:
    ...
    def create_dump(self, config: DumpConfig) -> str:
        """Create new dump"""
        with self.lock:
            dumper = MemsqlImplementation(config)
