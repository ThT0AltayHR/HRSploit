"""
MariaDB Database Dump Module - Dosya 54
...
logger = logging.getLogger('MariaDBDumper')
...
class MariaDBImplementation(MariaDBDumper):
    """Concrete implementation for MariaDB"""
    ...
    def get_tables(self) -> List[str]:
        """Get tables from MariaDB"""
        if not self.connection:
            return []
        
        tables = []
        try:
            # Get tables query for MariaDB
...
class MariaDBDumpManager:
    """Manage MariaDB dumps"""
    ...
    def create_dump(self, config: DumpConfig) -> str:
        """Create new dump"""
        with self.lock:
            dumper = MariaDBImplementation(config)
