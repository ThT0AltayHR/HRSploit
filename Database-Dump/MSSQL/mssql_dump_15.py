"""
MSSQL Database Dump Module - Dosya 15
...
logger = logging.getLogger('MSSQLDumper')
...
class MSSQLImplementation(MSSQLDumper):
    ...
    def get_tables(self) -> List[str]:
        """Get tables from MSSQL"""
        if not self.connection:
            return []
        
        tables = []
        try:
            # Get tables query for MSSQL
...
class MSSQLDumpManager:
    ...
    def create_dump(self, config: DumpConfig) -> str:
        """Create new dump"""
        with self.lock:
            dumper = MSSQLImplementation(config)
