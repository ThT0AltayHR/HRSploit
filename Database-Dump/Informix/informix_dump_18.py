"""
Informix Database Dump Module - Dosya 18
...
logger = logging.getLogger('InformixDumper')
...
class InformixImplementation(InformixDumper):
    """Concrete implementation for Informix"""
    
    def __init__(self, config: DumpConfig):
        super().__init__(config)
        self.db_type = 'Informix'
        
    def connect(self) -> bool:
        """Connect to Informix"""
        try:
            # Connection logic for Informix
    ...
    def get_tables(self) -> List[str]:
        """Get tables from Informix"""
        if not self.connection:
            return []
        
        tables = []
        try:
            # Get tables query for Informix
...
class InformixDumpManager:
    """Manage Informix dumps"""
    ...
    def create_dump(self, config: DumpConfig) -> str:
        """Create new dump"""
        with self.lock:
            dumper = InformixImplementation(config)
