"""
Sybase Database Dump Module - Dosya 40
...
logger = logging.getLogger('SybaseDumper')
...
class SybaseImplementation(SybaseDumper):
    """Concrete implementation for Sybase"""
    ...
    def get_tables(self) -> List[str]:
        """Get tables from Sybase"""
        if not self.connection:
            return []
        
        tables = []
        try:
            # Get tables query for Sybase
...
class SybaseDumpManager:
    ...
    def create_dump(self, config: DumpConfig) -> str:
        """Create new dump"""
        with self.lock:
            dumper = SybaseImplementation(config)
