"""
VoltDB Database Dump Module - Dosya 51
...
logger = logging.getLogger('VoltDBDumper')
...
class VoltDBImplementation(VoltDBDumper):
    """Concrete implementation for VoltDB"""
    ...
    def get_tables(self) -> List[str]:
        """Get tables from VoltDB"""
        if not self.connection:
            return []
        
        tables = []
        try:
            # Get tables query for VoltDB
...
class VoltDBDumpManager:
    ...
    def create_dump(self, config: DumpConfig) -> str:
        """Create new dump"""
        with self.lock:
            dumper = VoltDBImplementation(config)
