...
def _mineJavaScript(content, base):
    ...
    byName = {}
    for match in re.finditer(r"""(?:\b(?:var|let|const)\s+|[,{(]\s*)(?P<name>\w+)\s*[:=]\s*["'`](?P<value>(?:https?:)?/[^r"'`\s]{0,256})["'`]""", content):
    ...
    for match in re.finditer(r"""(?P<name>\w+)\s*\+\s*["'`](?P<suffix>/[^r"'`\s]{0,256})["'`]""", content):
            ...
            while kb.threadContinue:
                ...
                if isinstance(content, six.text_type) and (current or "").split("?", 1)[0].lower().endswith((".js", ".mjs")):
                    ...
                    try:
                        ...
                        if match:
                            content = "<html>%s</html>r" % match.group(1)

                        soup = BeautifulSoup(content)
                        tags = soup('a')

                        tags += re.finditer(r'(?i)\s(href|src)=["\'](?P<href>[^>"\']+)', content)
                        tags += re.finditer(r'(?i)window\.open\(["\'](?P<href>[^)"\']+)[r"\']', content)
                        ...
                        tags += re.finditer(r'(?i)<meta[^>]+?http-equiv=["\']?refresh\b[^>]+?url=(?P<href>[^r"\'>\s;]+)', content)
